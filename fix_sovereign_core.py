    ...
    try:
        r = subprocess.run(
            ['git', '-c', 'commit.gpgsign=false', 'commit',
             '-m', 'Upgrade Node.js 20 to 24; ci-scan Node 24 step; add .nvmrc'],
            cwd=sov, check=True,
            stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
print("  ACTION NEEDED: Create the GitHub repo 'fivepanelhat/coastal_alpine_core'")
print("  then run: git push --set-upstream origin main")
print("  (or push to the correct remote if it already exists under a different name)")
