...
class CoherenceReport(BaseModel):
    is_coherent: bool = Field(description="True if the entire monorepo is in perfect sync.")
...
def check_git_coherence(report: CoherenceReport):
    ...
    for status in statuses:
        ...
        if status.startswith('+'):
            ...
            report.submodule_sync_errors.append(f"State Drift Detected: {status.strip()}")
        elif status.startswith('-'):
            report.is_coherent = False
            report.submodule_sync_errors.append(f"Uninitialized Submodule: {status.strip()}")
...
def check_dependency_coherence(report: CoherenceReport, root_path: Path):
    ...
    target_repos = ['weaver', 'coastal_alpine_core', 'Blue-Moon-Portal', 'Sting-Operation-AI']
