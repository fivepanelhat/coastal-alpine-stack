...
def verify_package(update_file, signature_file):
 print(
 "[SECOPS] Auditing update signature against hardware root of trust..."
 )
 ...
 print(
 "[SUCCESS] Signature matches master keys. Package integrity validated."
 )
 return True
 else:
 print(
 "[CRITICAL UNTRUSTED PAYLOAD] Signature validation failed! Purging deployment asset."
 )
 return False
...
if __name__ == "__main__":
 if len(sys.argv) < 3:
 print(
 "Usage: python3 ota_verify.py <path_to_update_bin> <path_to_signature>"
 )
 sys.exit(1)
