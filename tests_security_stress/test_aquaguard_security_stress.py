...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../AquaGuard-Portal')))
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] AquaGuardSecurityTest: %(message)s")
...
def test_pydantic_constraints():
    ...
    try:
        valid_reading = WaterSensorReading(
            sensor_id="ph_sensor_1",
            sensor_type="pH",
            value=7.32,
            unit="pH"
        )
        logger.info(f"✓ Valid telemetry parsing works: id={valid_reading.sensor_id}, value={valid_reading.value}")
    ...
    try:
        ...
        logger.error("❌ SECURITY FAILURE: Accepted string value for float telemetry value!")
        raise AssertionError("Pydantic type constraint bypass!")
    except ValidationError:
        logger.info("✓ PASS: Correctly blocked string input on float telemetry value.")
    ...
    try:
        ...
        logger.error("❌ SECURITY FAILURE: Accepted invalid enum command for aerator ('super_high')!")
    ...
    try:
        ...
        logger.error("❌ SECURITY FAILURE: Accepted out-of-range confidence score (-0.5)!")
        raise AssertionError("Pydantic validation range bypass!")
    except ValidationError:
        logger.info("✓ PASS: Correctly blocked out-of-bounds confidence score (<0.0).")
...
async def test_pruner_stress():
    ...
    file_count = 100
    retention_hours = 2
    logger.info(f"Generating {file_count} dummy media files under {temp_media_dir}...")
    ...
    compliance_json = temp_compliance_dir / "audit_20260607_120000_aud-test.json"
    compliance_json.write_text(json.dumps({"audit_id": "aud-test", "status": "compliant"}), encoding="utf-8")
    ...
    logger.info(f"Initial stats: media_count={initial_stats['media_count']}, compliance_count={initial_stats['compliance_count']}")
    ...
    logger.info(f"Post cleanup stats: media_count={post_stats['media_count']}, compliance_count={post_stats['compliance_count']}")
    ...
    assert post_stats["media_count"] == 50, f"Expected 50 files remaining, but got {post_stats['media_count']}"
    assert post_stats["compliance_count"] == 1, "❌ SECURITY FAILURE: Compliance reports were pruned!"
    logger.info("✓ PASS: MediaPruner prunes old media files but preserves regional council compliance audits.")


if __name__ == "__main__":
    test_pydantic_constraints()
    asyncio.run(test_pruner_stress())
    logger.info("ALL AQUAGUARD PORTAL SECURITY AND STRESS TESTS PASSED SUCCESSFULLY!")
