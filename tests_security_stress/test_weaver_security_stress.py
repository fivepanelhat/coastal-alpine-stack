...
from weaver.langgraph.orchestrator import build_agnostic_helpdesk

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] WeaverSecurityTest: %(message)s")
...
def seed_knowledge_base():
    ...
    logger.info("✓ Seeding complete. Embedded 2 tenant-scoped compliance docs.")
...
def test_prompt_injection_guard():
    ...
    for prompt in malicious_prompts:
        ...
        outcome = "PASS" if (is_safe and expected == "Safe") or (not is_safe and expected == "Blocked") else "FAIL"
        logger.info(f"Prompt: '{prompt[:45]}...' -> Scanned: {'SAFE' if is_safe else 'BLOCKED'} (Expected: {expected}) -> {outcome}")
        assert outcome == "PASS", f"Injection guard failed for: {prompt}"
        
    logger.info("✓ PASS: Input guard correctly classified all adversarial prompts.")
...
def test_concurrent_stress():
    ...
    logger.info(f"Firing {concurrent_requests} concurrent query operations using ThreadPoolExecutor...")
    ...
    logger.info(f"Concurrency results:")
    logger.info(f"  Total Requests: {concurrent_requests}")
    logger.info(f"  Success Rate: {success_count}/{concurrent_requests} ({success_count/concurrent_requests*100:.1f}%)")
