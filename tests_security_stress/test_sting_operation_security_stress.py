...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] StingSecurityTest: %(message)s")
...
def validate_detection_box(x: float, y: float, w: float, h: float, score: float, img_size: int = 640) -> bool:
    """
...
def test_box_coordinate_guards():
    ...
    for x, y, w, h, score, expected in test_cases:
        ...
        logger.info(f"Box: [{x}, {y}, {w}, {h}], score={score} -> Validated: {is_valid} (Expected: {expected}) -> {outcome}")
        assert outcome == "PASS", f"Coordinate validation failed for: Box={x,y,w,h}"
        
    logger.info("✓ PASS: Coordinate guard filters out corrupted or adversarial bounding boxes.")
...
async def test_frame_ingestion_stress():
    ...
    logger.info(f"Simulating feed of {total_frames} camera frames under stress...")
    ...
    logger.info(f"  Frame Drop Rate: {processor.dropped_count / total_frames * 100:.1f}%")
    
    # Confirm it dropped frames rather than building memory backlogs
    assert processor.dropped_count > 0, "Stress loop did not activate frame dropping!"
    logger.info("✓ PASS: Frame drop strategy successfully prevented queue backing and out-of-memory states.")

if __name__ == "__main__":
    test_box_coordinate_guards()
    asyncio.run(test_frame_ingestion_stress())
    logger.info("ALL STING OPERATION AI SECURITY AND STRESS TESTS PASSED SUCCESSFULLY!")
