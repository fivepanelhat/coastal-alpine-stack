...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../Blue-Moon-Portal')))
...
from portal_schemas.ai_models import SensorReading, CropOptimizationPlan, PumpAction, LightingAction
from portal_core.media_pruner import MediaPruner
from pydantic import ValidationError

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] BlueMoonSecurityTest: %(message)s")
...
def test_pydantic_constraints():
    ...
    try:
        ...
        logger.error("❌ SECURITY FAILURE: Accepted string value for numerical sensor telemetry!")
    ...
    try:
        ...
        logger.error("❌ SECURITY FAILURE: Accepted invalid enum command ('super_high')!")
    ...
    try:
        ...
        logger.error("❌ SECURITY FAILURE: Accepted out-of-range confidence score (1.5)!")
        raise AssertionError("Pydantic validation range bypass!")
    except ValidationError:
        logger.info("✓ PASS: Correctly blocked out-of-bounds confidence score (>1.0).")
...
async def test_pruner_stress():
    ...
    logger.info(f"Initial storage count: {initial_stats['media_count']} files, size={initial_stats['total_size_mb']:.4f} MB")
    ...
    logger.info(f"Post cleanup storage count: {post_stats['media_count']} files.")
    ...
    logger.info("✓ PASS: Storage pruner successfully enforces storage lifecycle and releases disk space.")

if __name__ == "__main__":
    test_pydantic_constraints()
    asyncio.run(test_pruner_stress())
    logger.info("ALL BLUE MOON PORTAL SECURITY AND STRESS TESTS PASSED SUCCESSFULLY!")
