...
def get_owner_repo(repo_path):
    ...
    if os.path.isfile(config_path):
        with open(config_path, "r", encoding="utf-8") as cfg:
            for line in cfg:
                m = re.search(r"url = https://github.com/([^/]+)/([^/]+)(?:\\.git)?", line)
                if m:
...
def has_badge(content):
    pattern = re.compile(r"!\\[CI\\].*github\\.com/.*/actions/workflows/" + re.escape(WORKFLOW) + r"/badge\\.svg\\?branch=" + re.escape(BRANCH))
...
for entry in os.listdir(WORKSPACE):
    ...
    badge_md = BADGE_TEMPLATE.format(owner=owner, repo=repo, workflow=WORKFLOW, branch=BRANCH)
    readme_path = None
